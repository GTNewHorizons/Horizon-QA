...
Reports are attempted once in order: console, status JSON, then JUnit XML. A report-sink failure is added to the run result for later sinks and the process exit code, so JUnit describes any earlier reporting failure. If JUnit itself fails, no JUnit artifact can describe that failure.
...
Timing data is included in the status JSON at suite, test and step level. These measurements describe occupied wall time, not CPU time. Overlapping tests and nested intervals must not be added together to estimate suite duration. Simulation ticks and elapsed time stay separate even during acceleration.
...
While server ticks remain available, cancellation retains runner ownership until asynchronous instance cleanup finishes, then runs batch cleanup and writes the reports. An external client deadline only requests cancellation. The server thread owns cleanup and report publication. If the server is already stopping or has crashed, unfinished asynchronous cleanup is reported as `CLEANUP_INTERRUPTED` because its callbacks can no longer be drained through normal ticks. Synchronous cleanup is still attempted once. The run exits with infrastructure status `2`, and a server-stopping callback never starts a second process exit.
...
Client tests run a real Minecraft 1.7.10 client and integrated server. They need a working graphics context. Linux CI uses Xvfb, Mesa software OpenGL and the `xrandr` executable from `x11-xserver-utils`. LWJGL 2 invokes that executable to discover display modes. The launcher runs through Gradle on Windows and Linux, with JDK 17 or newer for build-side process supervision. Minecraft still runs on Java 8.
...
```sh
./gradlew -p scripts/client-tests runClientTests \
    --tests horizonqaexamples:ClientSmokeTests.clickEscapeAndReopen
```

The task creates a fresh report directory, starts the target project's `:examples:runClient` at 1280 by 720, selects the named tests, and enforces a 600-second wall-clock deadline. It writes `launch-result.json` with the existing exit codes: `0` for success, `1` for required test failure, and `2` for infrastructure failure or an incomplete run. Gradle itself returns a generic build failure for either nonzero result, so automation must inspect the JSON code. An intentional failure example is `horizonqaexamples:ClientSmokeTests.intentionalFailure`.

For a consumer checkout with Horizon QA already configured as a development dependency:
...
The standalone supervisor uses the consumer's Gradle wrapper and existing RFG task. It does not add dependencies or change consumer packaging. Keep client test classes out of normal published artifacts. During local framework development, `--horizon-qa-jar /path/to/horizonqa-dev.jar` forwards `-PhorizonQaJar` to a consumer that explicitly supports that override.

Use repeatable `--gradle-argument=-PusesMixinDebug=false` options for additional target build arguments. Each value remains one argument, without shell interpolation. Do not override client mode, selection, game directory, launch identity or report directory. The target build uses a private, single-use Gradle daemon so the supervisor never terminates a shared daemon.

`--report-dir` must name a new directory. `--timeout-seconds` bounds the whole launch including Gradle preparation. `--shutdown-seconds` defaults to 20. On expiry the supervisor requests orderly shutdown with a marker and attempts a bounded thread dump of the identified client. After the grace period, it terminates only the wrapper and descendants observed as belonging to that invocation. This also covers a build that hangs before Minecraft starts. Missing or malformed reports, incomplete execution and forced termination produce launch code `2`.

The same inputs are available as Gradle properties: `horizonQaTests`, `horizonQaProjectRoot`, `horizonQaClientTask`, `horizonQaClientReportDir`, `horizonQaTimeoutSeconds`, `horizonQaShutdownSeconds` and `horizonQaJar`. The supervisor and the isolation init script support configuration cache. `scripts/tests/portable-client-launcher/verify.py` exercises real owned processes without Minecraft, including startup hangs and an unrelated process that must remain alive.

The `Client tests` CI workflow boots real clients under Xvfb. It checks a passing GUI scenario, a late render failure, cancellation of an active run and two overlapping clients. It validates both report files, expected exit semantics and orderly process termination, and retains reports, logs, crash reports and screenshots on failure.

### Parallel client launches

Each launcher invocation uses `<ReportDir>/game` as its writable Minecraft directory. The companion Gradle init script sets the selected RFG task's `workingDir` before RFG calculates `--gameDir`. Configuration, options, logs and scratch saves therefore belong to one launch. `launch-result.json` records both `gameDir` and `launchId` alongside the existing result fields. The game directory starts fresh and does not copy settings or manually installed mods from the usual development run directory. Dependencies supplied by the build remain on its runtime classpath.

After the task's build dependencies finish, the init script snapshots classpath entries under the root project's and subprojects' configured build directories into `<ReportDir>/classpath`. This includes development JARs and classes/resource directories. The client uses those copies in the original classpath order, so subsequent rebuilds do not replace those artifacts underneath a running client. External dependency-cache entries remain shared. Outputs from independent included builds or other external file dependencies are outside this snapshot.

Run separate launcher invocations in parallel with distinct report directories, or omit `--report-dir` to generate one per invocation. Each process runs its selected batch serially through the existing runner. Distribute selectors between launches when splitting a suite. Launchers for the same project automatically serialize build preparation with a file lock, then release it when the classpath snapshot and game directory are ready. The next build can prepare while the previous game keeps running. Gradle retains its normal caches. Waiting for preparation counts toward the launch deadline.

This coordination covers launches through the portable launcher. Keep unsnapshotted external dependencies stable throughout the run, and do not delete an active report directory, including through `clean` when reports live under `build`. Unrelated build commands do not acquire the launcher's preparation lock.

Run `python scripts/tests/parallel-clients.py --report-dir build/client-parallel-check` to verify two real games together. Both instances of `ClientParallelTests.clickWhileAnotherClientIsRunning` open a screen and write their own readiness marker. The script waits for both live clients before releasing their independent GUI clicks, then verifies both results and distinct launch identities. The example requires its exact selector and is skipped during broader example runs.
...
The client-test runtime also mutes hostile creatures (`SoundCategory.MOBS`) and friendly or neutral mobs (`SoundCategory.ANIMALS`) for the whole automated run, including scenarios attached through `ClientTest`. This is an in-memory override of the effective volume. Saving options during a test preserves the configured values, and finishing the run removes the override. Other categories are unaffected by this runtime policy.
...
The reported run starts after the first world frame finishes rendering. World/player creation alone does not mark the client ready, so initial rendering does not consume the first scenario's execution or cleanup budget.
...
For a race fixture that must resize during an already pending click, use `ClientTest.requestWindowResize(width, height)` from an active client-thread callback. It requests the same native resize immediately and retains automatic restoration, but does not wait for the event or a frame. Calling it from the server thread or a closed session fails. The target resolver and readiness predicate must only observe state. Queueing `resizeWindow` from a resolver and waiting for its future deadlocks the FIFO queue because the active click stays ahead of the resize.

ClientTest has no separate render-callback registry. Register a one-shot `TickEvent.RenderTickEvent` listener on `FMLCommonHandler.instance().bus()` from a `scenario.client` setup action and unregister it with `c.afterTest(() -> bus.unregister(listener))`. At render END, observe that the target has been positioned, request the resize once, and let normal window events update the game. Keep click readiness false until that request has been sent and the new dimensions and actual hit test are ready. This exercises resize between positioning and dispatch without changing the queue's serial execution.
...
The session remains active until final capture completes. A render failure after `succeed()` but before teardown finishes changes the result to an infrastructure error and retains the original cause. `ClientLateRenderFailureTests.renderFailureAfterSuccessPreservesCauseAndCleansOnce` exercises this boundary. The three exact `ClientCancellationTests` IDs exercise an external deadline after one passed case, verifying client cleanup, server cleanup and batch cleanup in order, with the final case remaining unstarted. These examples require exact selection because they intentionally terminate their client process with exit code `2`.

An integrated-server tick crash aborts the reported run with exit code `2`, retaining the original cause and the cases already observed. Unloading the test world also ends the client run as an infrastructure failure. These conditions stop further scenarios and close the test client without waiting for the launcher's external deadline. Ordinary assertion failures still follow the normal required/optional failure policy and allow the next test after successful cleanup.

`ClientLifecycleTests.clientLinkageFailureAbortsRun` and `ClientLifecycleTests.worldUnloadAbortsRun` exercise these terminal paths. Run each by its exact full test ID in a separate client process and expect exit code `2`. They skip in broader selections because deliberately terminating the world would prevent the remaining examples from running.
